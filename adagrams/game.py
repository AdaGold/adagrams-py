...
def draw_letters():
    ...
    while len(letter_bank) < 10:
        ...
        if random_letter not in tiles:
            tiles[random_letter] = 1
            letter_bank.append(random_letter)
        elif tiles[random_letter] < LETTER_POOL[random_letter]: 
            tiles[random_letter] += 1
            letter_bank.append(random_letter)
...
def uses_available_letters(word, letter_bank):
    ...
    for letter in set(capital_word):
        if letter not in letter_bank or count_items(letter, capital_word) > count_items(letter, letter_bank):
            return False
    return True
